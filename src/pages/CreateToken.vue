...
<template>
  ...
  <div class="events">
    <span v-if="deploying">
      ...
      <strong>Please confirm all transactions in your browser extension, and wait until they are added to the
        Blockchain.</strong>
    </span>
    ...
    <div v-for="(event, index) in deployEvents" :key="index">
      <span v-if="event.type === 'PROXY_DEPLOYMENT'">
        Contract deployed: {{ event.contractName }} ({{ event.type }}): <a
          :href="'https://blockscout.com/lukso/l14/address/' + event.contractAddress" target="_blank">{{
              event.contractAddress
          }}</a><br>
        Transaction hash: <a :href="'https://blockscout.com/lukso/l14/tx/' + event.receipt.transactionHash"
          target="_blank">{{ event.receipt.transactionHash }}</a>
      </span>
      <br>
